...
SWE-bench Lite was attempted as an external agent pilot, but it produced empty patches and no resolved tasks. We removed it from the maintained result surface and use controlled coding-drift plus retrieval/drifted retrieval for the final paper.
...
- OpenRouter-backed DSPy ReAct is the maintained execution path.
...
- `uv run --extra dev --extra dspy --extra openai python -m pytest -q tests/test_benchmark_cli.py tests/test_benchmark_report.py tests/test_benchmarks.py tests/test_verification.py tests/test_retrieval.py tests/test_types.py`
