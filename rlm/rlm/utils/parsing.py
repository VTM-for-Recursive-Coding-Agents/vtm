...
DEFAULT_ASSISTANT_HISTORY_CHAR_LIMIT = 6000


def _truncate_text(text: str, max_character_length: int) -> str:
    if len(text) <= max_character_length:
        return text
    return text[:max_character_length] + f"... + [{len(text) - max_character_length} chars...]"


def _compress_iteration_response(
    response: str,
    ...
    compressed = response
    if code_block_count:
        compressed = re.sub(r"```repl\s*\n.*?\n```", "", compressed, flags=re.DOTALL)
        compressed = re.sub(r"\n{3,}", "\n\n", compressed).strip()
        note = (
            f"[Executed {code_block_count} REPL block"
            f"{'s' if code_block_count != 1 else ''}; see REPL transcript below.]"
        )
        compressed = f"{compressed}\n\n{note}" if compressed else note
    return _truncate_text(compressed, max_character_length)
    ...
    if match:
        ...
        if environment is not None:
            ...
            if (
                ...
                return None
            # Don't treat a Python callable/object repr as a valid answer — the model
            # accidentally passed a function rather than calling it (e.g. FINAL_VAR(solve)
            # where solve is a def).  Return None so the loop continues and the model can
            # call the function and store the result in a plain string variable first.
            if re.match(r"^<(function|class|method|module|built-in function|bound method)\s", final_answer):
                return None
            return final_answer
        return None
...
def format_iteration(
    ...
    assistant_max_character_length: int = DEFAULT_ASSISTANT_HISTORY_CHAR_LIMIT,
) -> list[dict[str, str]]:
    """
    ...
    """
    assistant_content = _compress_iteration_response(
        iteration.response,
        len(iteration.code_blocks),
        assistant_max_character_length,
    )
    ...
    for code_block in iteration.code_blocks:
        ...
        result = _truncate_text(result, max_character_length)
