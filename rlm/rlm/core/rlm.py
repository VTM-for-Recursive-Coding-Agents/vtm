...
class RLM:
    ...
    def __init__(
        ...
        Args:
            ...
            max_prompt_tokens: Maximum input prompt tokens allowed for any single LM request. Execution stops
                before a backend request if the actual message payload exceeds this limit.
        ...
        self.max_prompt_tokens = max(1, max_prompt_tokens) if max_prompt_tokens is not None else None
    ...
    def _completion_metadata(
        self, lm_handler: LMHandler | None = None, client: BaseLM | None = None
    ) -> dict[str, Any] | None:
        if self.logger:
            return self.logger.get_trajectory()

        source_client = client
        if source_client is None and lm_handler is not None:
            source_client = lm_handler.default_client

        metadata = getattr(source_client, "last_response_metadata", None)
    ...
    def completion(
        ...
        with self._spawn_completion_context(prompt) as (lm_handler, environment):
            ...
            self._check_prompt_context_limit(message_history, lm_handler=lm_handler)
                ...
                for i in range(self.max_iterations):
                    ...
                    if final_answer is not None:
                        ...
                        return RLMChatCompletion(
                            ...
                            metadata=self._completion_metadata(lm_handler=lm_handler),
                        )
    ...
    def _completion_turn(
        ...
        self._check_prompt_context_limit(prompt, lm_handler=lm_handler)
    ...
    def _default_answer(self, message_history: list[dict[str, Any]], lm_handler: LMHandler) -> str:
        ...
        self._check_prompt_context_limit(current_prompt, lm_handler=lm_handler)
    ...
    def _fallback_answer(self, message: str | dict[str, Any]) -> str:
        ...
        """
        client: BaseLM = get_client(self.backend, self.backend_kwargs)
        self._check_prompt_context_limit(message, client=client)
        response = client.completion(message)
        return response

    def _prompt_messages(self, prompt: str | dict[str, Any]) -> list[dict[str, Any]]:
        if isinstance(prompt, list):
            return [dict(message) for message in prompt]
        return [{"role": "user", "content": prompt}]

    def _check_prompt_context_limit(
        self,
        prompt: str | dict[str, Any],
        lm_handler: LMHandler | None = None,
        client: BaseLM | None = None,
    ) -> None:
        source_client = client or (lm_handler.default_client if lm_handler is not None else None)
        if source_client is None or not source_client.model_name:
            return

        messages = self._prompt_messages(prompt)
        prompt_tokens = count_tokens(messages, source_client.model_name)
        context_limit = get_context_limit(source_client.model_name)
        safe_prompt_limit = max(1, context_limit - PROMPT_TOKEN_SAFETY_MARGIN)
        prompt_limit = safe_prompt_limit
        limit_reason = (
            f"safe context window for model {source_client.model_name} "
            f"(raw context limit {context_limit:,})"
        )

        if self.max_prompt_tokens is not None and self.max_prompt_tokens < prompt_limit:
            prompt_limit = self.max_prompt_tokens
            limit_reason = (
                f"configured prompt budget for model {source_client.model_name} "
                f"({self.max_prompt_tokens:,} tokens; raw context limit {context_limit:,})"
            )
        ...
        if prompt_tokens > prompt_limit:
            raise TokenLimitExceededError(
                ...
                partial_answer=self._best_partial_answer,
                message=(
                    f"Prompt exceeds {limit_reason}: {prompt_tokens:,} input tokens vs "
    ...
    def _subcall(self, prompt: str, model: str | None = None) -> RLMChatCompletion:
        ...
        if next_depth >= self.max_depth:
            ...
            try:
                self._check_prompt_context_limit(prompt, client=client)
                ...
                return RLMChatCompletion(
                    ...
                    metadata=self._completion_metadata(client=client),
                )
