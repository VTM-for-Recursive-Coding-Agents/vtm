...
class TestFormatIteration:
    ...
    def test_strips_repl_blocks_from_assistant_history(self):
        code_result = REPLResult(stdout="3", stderr="", locals={"x": 3})
        iteration = RLMIteration(
            prompt="Calculate 1+2",
            response="Let me calculate that.\n```repl\nx = 1 + 2\nprint(x)\n```",
        ...
        messages = format_iteration(iteration)
        assert "```repl" not in messages[0]["content"]
        assert "Executed 1 REPL block" in messages[0]["content"]
    ...
    def test_truncates_long_assistant_history(self):
        iteration = RLMIteration(
            prompt="Test",
            response="x" * 1000,
            code_blocks=[],
        )
        messages = format_iteration(iteration, assistant_max_character_length=100)
        assert len(messages[0]["content"]) < 1000
        assert "chars" in messages[0]["content"]
