#!/usr/bin/env bash
#Generated by Copilot, edited by Alexander Fraser, this files checks that repositories for benchmarks are in place and that required commands are available before running benchmarks. This is intended to be run by CI and can also be used locally to verify setup before running benchmarks.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: scripts/preflight_checks.sh [--benchmark all|lcb|swe]

Checks required tooling and repository layout before running benchmark pipelines.
EOF
}
...
echo "[preflight] Checking common requirements"
check_command git
